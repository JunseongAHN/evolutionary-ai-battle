...
| **K1 `race_v2`** | racer 정확조준, auto-pickup, **assist 없음** | **4M** | 12.0 s | 0.08/0.16 | 18.7 | 0.06 | 7.8 | **0** | 0.78 | 스텝만 늘린 대조군: 조준을 못 하니 4M도 무의미 |
| **K2** = K1 시드 2 | | 4M | 12.2 s | 0.10/0.21 | 19.4 | 0.05 | 8.5 | **0** | 0.77 | 재현됨 |
| **K3 `race_v2`** | **racer 10°, auto-pickup, aim assist** | **4M** | **32.0 s** | 0.09/0.17 | 25.9 | **0.26** | **69.0** | **0.38** | 0.75 | 줍고 싸운다, 레이스는 여전히 안 함 |
| **K4** = K3 시드 2 | | 4M | **33.6 s** | 0.09/0.18 | 25.1 | **0.28** | **75.0** | **0.42** | 0.75 | 재현됨 |
| **n1 `race_v2_cap2`** | K3와 동일, `capture` 2.0 | 4M | 33.5 s | 0.13/0.27 | 27.8 | 0.23 | 67.9 | 0.35 | 0.74 | §4의 처방. 캡처 가중치도 지렛대가 아니다 |
...
### K1–K4 결론 (2026-09-10, 4×3090 Linux 서버, Docker)

**스텝이 아니라 원시 동작의 비대칭이 원인이었다** — 이 트랙이 물어보려던 것에 답이 나왔다. K1–K2는 K0의 13배(300k → 4M)를
돌렸는데도 `kills`가 **정확히 0**이고 데미지 8, 명중률 0.06에 머문다. 같은 스텝의 K3–K4는 데미지 9배, 명중률 4.5배,
생존 2.7배이고 킬이 실제로 난다. 시드 2개가 같은 행동이라 재현도 됐다. 반면 `team_captures`는 네 run 모두 0.16–0.21로
동일하다 — 조준을 고쳐도 레이스는 나오지 않는다. §4 판정 표의 "싸우지만 레이스 안 함" 분기이므로 처방은 `capture` 2.0
한 시드(`configs/race_v2_cap2.json`)뿐이고, 그 이상 이 트랙에 사람 시간을 쓰지 않는다.

### `capture` 2.0 (run n1) — §4 처방의 결론

§4 판정 표가 "싸우지만 레이스 안 함" 분기에 지시한 유일한 조치를 실행했다(`configs/race_v2_cap2.json`,
4M step, K3와 나머지 설정 동일). 결과: `team_captures` 0.17/0.18 → **0.27**. 기준값 1.5의 6분의 1이고,
생존(33.5 vs 32.0/33.6) · 데미지(67.9 vs 69.0/75.0) · kills(0.35 vs 0.38/0.42)는 두 시드의 산포 안에서
움직이지 않는다. **캡처 가중치도 지렛대가 아니다.** 계획대로 이 트랙에 사람 시간을 더 쓰지 않는다 —
필요한 것은 "강도 조절이 되는 상대"였고 그건 아래 표로 확보됐다.

### 강도 기준선 (§5 eval, 각 50 에피소드 / random 20)

이 트랙의 실제 산출물이다: 강도 노브가 달린 상대와, 하네스 비교에 그대로 넣을 metric vector.

| 정책 | 상대 | 생존 | hp_mean | 데미지(줌/받음) | kills | 명중률 | team_cap | armed |
|---|---|---|---|---|---|---|---|---|
| K3 | racer 10° (학습 상대) | 28.6 s | 67.2 | 63.6 / 115.4 | 0.35 | 0.27 | 0.16 | 0.77 |
| K3 | racer 정확조준 (전이) | 15.9 s | 85.8 | 41.8 / 129.0 | 0.12 | 0.24 | 0.20 | 0.75 |
| K3 | chaser 정확조준 | 9.2 s | 83.0 | 38.2 / 141.9 | 0.12 | 0.46 | 0.06 | 0.78 |
| K4 | racer 10° (학습 상대) | 34.9 s | 67.0 | 90.3 / 103.9 | 0.45 | 0.31 | 0.14 | 0.75 |
| K4 | racer 정확조준 (전이) | 18.3 s | 83.0 | 56.1 / 124.8 | 0.22 | 0.30 | 0.14 | 0.74 |
| K4 | chaser 정확조준 | 11.3 s | 82.3 | 39.7 / 136.2 | 0.12 | 0.45 | 0.04 | 0.73 |
| random | racer 정확조준 | 7.0 s | 88.0 | 0.7 / 145.5 | 0.00 | 0.04 | 0.00 | 0.12 |

읽는 법. **강도 축이 작동한다**: 정확 조준 racer는 10° racer보다 생존을 절반으로 깎고 kills를 3분의 1로 줄인다 —
10°에서 배운 것의 절반쯤이 전이되며, K0에서 본 비율이 4M에서도 유지된다. **chaser가 생존에는 가장 어려운 상대**다
(9–11 s): 거리를 좁혀 오므로 명중률은 오히려 올라가지만(0.45) 받는 데미지가 가장 크다. random 바닥선은 생존 7 s ·
데미지 0.7 · armed 0.12이므로 K3/K4는 바닥선 대비 생존 4–5배, 데미지 90–130배다. `team_captures`는 어느 조합에서도
0.04–0.20으로, 학습 로그와 같은 결론(레이스 미학습)을 eval에서도 확인한다. racer 상대 `win_rate`가 0인 것은 race 모드가
`endOnElimination: false`라 대부분 시간 제한까지 가고 승패가 캡처 수로 갈리기 때문이다 — 캡처를 안 하니 이길 수 없다.

주의: `eval`에는 `--objective` 플래그가 없다(체크포인트 meta에서 복원한다). 그래서 `--policy random` 바닥선은 목표 없이
돌고 `team_captures`가 구조적으로 0이다 — 캡처는 학습 정책끼리만 비교할 것.
